...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(epoched,epochs)):
        ...
        for batch in tqdm(train_loader):
            ...
            batch[img_index], d_m  = utils.rand_drop_channel(modalities_to_train,
                                                                    batch[img_index],
                                                                    mode="modmean")
        ...
        if epoch % Training_config.val_interval == 0: # and epoch > 400 :# not epoch == 0 and
            ...
            with torch.no_grad():
                ...
                for val_data in val_loader:
                    ...
                    complete_modality_image = val_input.to(device)
                    val_labels = val_data[1].to(device)                        
                    roi_size = (cropped_input_size[0], cropped_input_size[1], cropped_input_size[2])
                    sw_batch_size = 1
                    #using sliding window for the whole 3D image
                    c_logits = sliding_window_inference(complete_modality_image, roi_size=(128,128,128), sw_batch_size=1, predictor = lambda x: model(x, modalities_dropped_info=[]))
                    ...
                    for r in range(1, 4):       
                        for channels_to_drop_tuple in itertools.combinations(range(4), r):  # These are the indices to drop  # Generate all valid subsets
                            ...
                            missing_modality_image = utils.drop_modality_image_channel(complete_modality_image,
