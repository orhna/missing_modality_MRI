...
if __name__ == "__main__":
    ...
    train_loader, val_loader = utils.get_loaders_192(images=images,
                                                segs=segs,
    ...
    if Training_config.model_type == "SwinUNETR":
        ...
        if not model_sharding:
            ...
            model = Multimodal_SwinUNETR(
                ...
                tp_conv=Training_config.tp_conv).to(device)
        else:
    ...
    for epoch in tqdm(range(epoched,epochs)):
        ...
        for batch in tqdm(train_loader):
            ...
            _w = 0.2
            ds_loss = ds_loss_0 + ds_loss_1 + ds_loss_2 + ds_loss_3 + ds_loss_4
            total_loss = loss + _w * ds_loss + _w * sep_losses
            
            if Training_config.tp_conv:
                tp_conv_loss = 0.25* torch.stack(tp_conv_losses).sum()
