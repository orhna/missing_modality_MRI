...
from nets.mm_3Dunet import mm_3Dunet, mm_3DUnetWrapper, drop_modality_image_channel, extract_complete_modality_features
from nets.fen import ContrastiveLoss, CosineContrastiveLoss
...
if __name__ == "__main__":
    ...
    criterionContrastive = CosineContrastiveLoss()
    ...
    for epoch in tqdm(range(epoched,epochs)):
        ...
        for batch in tqdm(train_loader):
            ...
            anchor = rec_model(missing_modality_features)# [0].unsqueeze(0)
            pos = rec_model(complete_modality_features)# [0].unsqueeze(0)
            #neg = rec_model(complete_modality_features[1].unsqueeze(0)) 

            loss_contrastive = criterionContrastive(anchor,pos)#,neg
        ...
        epoch_loss_contrastive /= step

        epoch_total_loss = epoch_loss_contrastive
        writer.add_scalar("Training/EpochTotalLoss", epoch_total_loss, epoch)
        writer.add_scalar("Training/EpochContrastiveLoss", epoch_loss_contrastive, epoch)
