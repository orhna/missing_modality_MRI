...
if __name__ == "__main__":
    ...
    log_dir = f"./logs/ldm_decide/{Training_config.experiment_name}_{_date}"
    ...
    ldm_model = mm_3Dldm(Training_config, device=device)
    ...
    for epoch in tqdm(range(epoched,epochs)):
        ...
        if not epoch == 0 and epoch % 5 == 0:            
            ...
            torch.save(wrapper.ldm_model.state_dict(), model_save_name)
            torch.save({
            'epoch': epoch,
            'model_state_dict': wrapper.ldm_model.state_dict(),
        ...
        if not epoch == 0 and epoch % Training_config.val_interval == 0: # 
            wrapper.ldm_model.eval()
            wrapper.ldm_model.ldm_eval = True
