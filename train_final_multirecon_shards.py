...
from nets.multimodal_swinunetr_multirecon_shards import Multimodal_SwinUNETR 
...
if __name__ == "__main__":
    ...
    devices = utils.initialize_GPUs(device_list)
    ...
    for epoch in tqdm(range(epoched,epochs)):
        ...
        for batch in tqdm(train_loader):
            optimizer.zero_grad()
            step += 1
            label = batch[label_index].to(device_list[1])
        ...
        # save model
        if not epoch == 0 and epoch % 5 == 0 and epoch > 400:            
        ...
        # validation
        if epoch % Training_config.val_interval == 0 and epoch > 400:# not epoch == 0 and
            model.eval()
            ...
            with torch.no_grad():
                ...
                for val_data in val_loader:
                    ...
                    complete_modality_image = val_input.to(device_list[1]) 
                    val_labels = val_data[1].to(device_list[1])                      
                    roi_size = (cropped_input_size[0], cropped_input_size[1], cropped_input_size[2])
                    sw_batch_size = 1
                    #using sliding window for the whole 3D image
                    c_logits = sliding_window_inference(complete_modality_image,
                    ...
                    for r in range(1, 4):       
                        for channels_to_drop_tuple in itertools.combinations(range(4), r):  # These are the indices to drop  # Generate all valid subsets
                            modalities_to_drop_list = list(channels_to_drop_tuple) # e.g. [1, 3]
                            ...
                            missing_modality_image = drop_modality_image_channel(complete_modality_image,
                                                                                ...
                                                                                idx_to_drop = modalities_to_drop_list,
                                                                                remaining_modalities = remaining_channels )
                            m_logits = sliding_window_inference(missing_modality_image,
                                                                 roi_size=(128,128,128),
                                                                   sw_batch_size=1,
                                                                     predictor= lambda x: model(x, modalities_dropped_info=modalities_to_drop_list),
                                                                      device=device_list[1])
                ...
                utils.log_visuals(modalities_to_train, complete_modality_image, c_outputs[0], val_labels, epoch, writer, chosen_ds)
