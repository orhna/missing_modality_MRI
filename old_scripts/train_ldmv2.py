...
from nets.ldm import ldm, LDMWrapper
from nets.mm_ldm import mm_ldm, mm_LDMWrapper
from nets.mm_ldmv2 import mm_ldmv2, mm_LDMWrapperv2, random_modality_channel
...
if __name__ == "__main__":
    ...
    log_dir = f"./logs/ldm/{Training_config.experiment_name}_{_date}"
    ...
    #ldm_model = mm_ldm(Training_config, device_list=device_id_list)
    #ldm_model.load_swinunetr_weights(load_model_path)
    #wrapper = mm_LDMWrapper(ldm_model)
    ldm_model = mm_ldmv2(Training_config, device_list=device_id_list)
    ldm_model.load_swinunetr_weights(load_model_path)
    wrapper = mm_LDMWrapperv2(ldm_model)
    ...
    for epoch in tqdm(range(epoched,epochs)):
        ...
        wrapper.ldm_model.train()
        print("-" * 10)
        print(f"epoch {epoch + 1}/{epochs}")
        epoch_diff_loss = 0
        epoch_scenario_loss = 0
        step = 0
        ...
        for batch in tqdm(train_loader):
            ...
            diff_loss, loss_dict = wrapper.ldm_model(complete_modality)
            scenario_loss = 0
            for loss in loss_dict.values():
                scenario_loss += loss/14

            total_loss = diff_loss + scenario_loss
            total_loss.backward()
            optimizer.step()
            
            print(f"Diffusion loss: {diff_loss.item():.4f}")
            batch_total_loss = diff_loss.item() + scenario_loss.item()
            writer.add_scalar("Training/BatchTotalLoss", batch_total_loss, step + epoch*epoch_len)
            writer.add_scalar("Training/BatchDiffLoss", diff_loss.item(), step + epoch*epoch_len)
            epoch_diff_loss += diff_loss.item()
            epoch_scenario_loss += scenario_loss.item()

        epoch_diff_loss /= step
        epoch_scenario_loss /= step

        epoch_total_loss = epoch_diff_loss + epoch_scenario_loss
        ...
        writer.add_scalar("Training/EpochScenarioMSE", epoch_scenario_loss, epoch)
        ...
        if not epoch == 0 and epoch % Training_config.val_interval == 0: # 
            ...
            with torch.no_grad():
                ...
                for val_data in val_loader:
                    ...
                    random_mod_channel = random_modality_channel(Training_config.drop_mode,
                                                                 complete_modality_image[0][0],
                                                                 device_list)
