...
from nets.mm_3Dunet import mm_3Dunet, mm_3DUnetWrapper_swinvit, drop_modality_image_channel, extract_features_from_swinvit, drop_modality_feature_channel
...
if __name__ == "__main__":
    ...
    log_dir = f"./logs/recon_swinvit/{Training_config.experiment_name}_{_date}"
    ...
    for epoch in tqdm(range(epoched,epochs)):
        ...
        for batch in tqdm(train_loader):
            ...
            complete_modality_features = extract_features_from_swinvit(rec_model.swinunetr,
                                                                        complete_modality_image,
                                                                        Training_config.recon_level)
            missing_modality_features = extract_features_from_swinvit(rec_model.swinunetr,
            ...
            """
            with torch.no_grad(): 
                c_f_m_generated_logits = rec_model.swinunetr(missing_modality_image, reconstructed=[recon_complete_modality,channels_to_drop])
                #m_logits = rec_model.swinunetr(missing_modality_image)
            """
            
            #cfm_dice_loss = criterionDiceCE(c_f_m_generated_logits, label) 
            #m_dice_loss = m_criterionDiceCE(m_logits, label) 
            #print(f"cfm dice loss: {cfm_dice_loss.item():.6f}")
            #print(f"m dice loss: {m_dice_loss.item():.6f}")
            
            total_loss = mse_loss #0.2 * mse_loss + 0.8 * cfm_dice_loss  
            total_loss.backward()
            optimizer.step()
            ...
            #epoch_dice_loss += cfm_dice_loss.item()
                ...
                for val_data in tqdm(val_loader):
                    ...
                    for r in range(1, 4):       
                        for channels in itertools.combinations(range(4), r):  # Generate all valid subsets
                            ...
                            del m_logits, c_f_m_generated_logits
                            ...
                            del m_outputs, c_f_m_generated_outputs
