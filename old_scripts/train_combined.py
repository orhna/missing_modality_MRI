...
if __name__ == "__main__":
    ...
    log_dir = f"./logs/combined/{Training_config.experiment_name}_{_date}"
    ...
    # load pre-trained SwinUNETR on BRATS21
    model_dict = torch.load(Training_config.path21)["state_dict"]
    swinunetr_model.load_state_dict(model_dict)
    source_state_dict = swinunetr_model.state_dict()
    ...
    target_state_dict = utils.load_n_duplicate_mm_weights(source_state_dict, target_state_dict)
    ...
    for epoch in tqdm(range(epoched,epochs)):
        ...
        for batch in tqdm(train_loader):
            ...
            if Training_config.random_modality_drop:
                batch[img_index], d_m  = utils.rand_drop_channel_new(modalities_to_train, batch[img_index], Training_config.drop_mode)
